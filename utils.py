...
def listar():
    ...
    produtos = cursor.fetchall();'#O retorno do resultado será passado como um lista'
    if len(produtos) > 0:
...
def inserir():
    ...
    sql = (f"INSERT INTO produtos (nome, preco, estoque) VALUES ('{nome}', {preco}, {estoque})")
    cursor.execute(sql)
...
def atualizar():
    ...
    sql = (f"UPDATE produtos SET nome='{nome}', preco={preco}, estoque={estoque} WHERE id={codigo}")
    cursor.execute(sql)
    connection.commit()
